...
def extractTimestampFromISO8601 (isostring):
	...
	no timezone as we use UTC everywhere
	:param isostring: source string as YYYY-MM-DDTHH:MM:SSZ
	:return:
	"""

	regex = re.compile(r"\d{2}")
	datestring, timezonestring = isostring.split("T")
	datefields = regex.findall(datestring)
	timefields = regex.findall(timezonestring)

	return int(datefields[0]+datefields[1]+datefields[2]+datefields[3]+timefields[0]+timefields[1]+timefields[2])
